...
$StdoutLog = Join-Path $LogDir 'tia-http.stdout.log'
$StderrLog = Join-Path $LogDir 'tia-http.stderr.log'
$PidFile = Join-Path $LogDir 'tia-http.pid'

if (-not (Test-Path -LiteralPath $ExePath)) {
    throw "未找到一期 runtime：$ExePath"
}
...
if ($existing) {
    ...
    Write-Host "一期 MCP 服务已在运行：PID=$($pids -join ', ')"
...
try {
    ...
    if ($connect.Wait(600) -and $tcp.Connected) {
        throw "端口 $Port 已被其他进程占用，未启动一期服务。请先停止旧 MCP 服务或换用 TIA_MCP_HTTP_PORT。"
    }
}
finally {
    $tcp.Dispose()
...
$env:TIA_MCP_TOOL_PROFILE = 'plc-software-v17'

$args = @(
    '--transport', 'http',
    '--http-prefix', $prefix,
    '--http-api-key', $ApiKey,
    '--tool-profile', 'plc-software-v17',
...
Write-Host "一期 MCP 服务已启动：PID=$($process.Id)，地址=$prefix，profile=plc-software-v17"
