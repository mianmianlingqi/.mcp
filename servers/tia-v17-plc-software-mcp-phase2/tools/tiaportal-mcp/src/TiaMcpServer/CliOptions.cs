namespace TiaMcpServer
{
    public class CliOptions
    {
        ...
        public int? HttpResponseTimeoutSeconds { get; set; } // local HTTP bridge response timeout for long TIA operations
        public string? ToolProfile { get; set; } // optional tool surface profile, e.g. "plc-software-v17"
        ...
        public static CliOptions ParseArgs(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                switch (args[i].ToLowerInvariant())
                {
                    ...
                    case "--http-response-timeout-seconds":
                        if (i + 1 < args.Length && int.TryParse(args[i + 1], out int httpResponseTimeoutSeconds))
                        {
                            options.HttpResponseTimeoutSeconds = httpResponseTimeoutSeconds;
                            i++;
                        }
                        break;
