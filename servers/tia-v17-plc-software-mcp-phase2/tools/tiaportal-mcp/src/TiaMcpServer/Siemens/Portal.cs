...
namespace TiaMcpServer.Siemens
{
    public partial class Portal
    {
...
#if TIA_V20 || TIA_V17
            // V20/V17 do not expose Siemens.Engineering.HW.CommunicationConnections. The hardware-level
        ...
        public ResponseMessage EnsureWatchTableAddressEntry(
            string softwarePath,
            string tableName,
            string address)
        {
            if (IsProjectNull()) return new ResponseMessage { Message = "No project open.", Meta = new JsonObject { ["success"] = false } };
            var plc = GetPlcSoftware(softwarePath);
            if (plc == null) return new ResponseMessage { Message = $"PLC software not found: '{softwarePath}'.", Meta = new JsonObject { ["success"] = false } };

            try
            {
                var group = ResolvePlcWatchAndForceTableGroup(plc);
                if (group == null) return new ResponseMessage { Message = "WatchAndForceTableGroup not accessible.", Meta = new JsonObject { ["success"] = false } };

                var table = FindOrCreateWatchTable(group, tableName);
                if (table == null) return new ResponseMessage { Message = $"Could not find or create watch table '{tableName}'.", Meta = new JsonObject { ["success"] = false } };

                var entry = FindOrCreateTableEntry(table, "Entries", address);
                if (entry == null) return new ResponseMessage { Message = $"Could not create entry for address '{address}'.", Meta = new JsonObject { ["success"] = false } };

                TrySetProperty(entry, "Address", address);

                return new ResponseMessage
                {
                    Message = $"Watch table '{tableName}': offline address entry '{address}' ensured.",
                    Meta = new JsonObject
                    {
                        ["success"] = true,
                        ["softwarePath"] = softwarePath,
                        ...
                        ["address"] = address,
                        ["offlineOnly"] = true,
                        ["writesPlcValue"] = false,
                        ["setsModifyValue"] = false,
                        ["setsForceValue"] = false
                    }
                };
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "EnsureWatchTableAddressEntry failed");
                return new ResponseMessage
                {
        ...
        private static object? FindOrCreateTableEntry(object table, string entriesPropertyName, string address)
        {
            ...
            var created = TryInvokeMethodByName(entries, "Create", address)
                ?? TryInvokeMethodByName(entries, "Create");
            if (created != null)
            {
                TrySetProperty(created, "Address", address);
                return created;
            }
        ...
        /// <summary>创建一个工艺对象（Technology Object）。底层调用 TechnologicalInstanceDBComposition.Create()。</summary>
        public ResponseMessage CreateTechnologyObject(
            string softwarePath,
            string name,
            string systemLibElement,
            string libVersion)
        {
            if (IsProjectNull()) return new ResponseMessage { Message = "No project open." };
            var plc = GetPlcSoftware(softwarePath);
            ...
            try
            {
                var group = TryGetPropertyValue(plc, "TechnologicalObjectGroup", "TechnologyObjectGroup");
                if (group == null)
                    return new ResponseMessage { Message = "TechnologicalObjectGroup not found." };

                var tos = TryGetPropertyValue(group, "TechnologicalObjects", "TechnologyObjects", "Instances", "Objects");
                if (tos == null)
                    return new ResponseMessage { Message = "Technology objects collection not found. groupType=" + group.GetType().FullName };

                var createMethod = tos.GetType().GetMethod("Create");
                if (createMethod == null)
                    return new ResponseMessage { Message = "Create method not found on TO collection. type=" + tos.GetType().FullName };

                Version ver;
                try { ver = new Version(libVersion); }
                catch
                {
                    return new ResponseMessage
                    {
                        Message = $"Invalid version format: '{libVersion}'. Use 'X.Y' format (e.g. '2.3').",
                        Meta = new JsonObject { ["success"] = false }
                    };
                }

                var result = createMethod.Invoke(tos, new object[] { name, systemLibElement, ver });
                if (result == null)
                    return new ResponseMessage { Message = "Create returned null.", Meta = new JsonObject { ["success"] = false } };

                var meta = new JsonObject
                {
                    ["success"] = true,
                    ["name"] = TryGetPropertyValue(result, "Name")?.ToString() ?? name,
                    ["type"] = systemLibElement,
                    ["version"] = libVersion,
                    ["softwarePath"] = softwarePath
                };
                ...
                return new ResponseMessage
                {
                    Message = $"Technology object '{name}' ({systemLibElement} V{libVersion}) created in '{softwarePath}'.",
                    Meta = meta
                };
            }
            catch (TargetInvocationException tie) when (tie.InnerException != null)
            {
                return new ResponseMessage
                {
                    Message = $"Create failed: {tie.InnerException.Message}",
                    Meta = new JsonObject { ["success"] = false, ["error"] = tie.InnerException.Message }
                };
            }
            catch (Exception ex)
            {
                return new ResponseMessage
                {
                    ...
                    Meta = new JsonObject { ["success"] = false, ["error"] = ex.Message }
                };
            }
        }
        ...
        public List<ModelContextProtocol.CrossReferenceEntry>? GetCrossReferences(string softwarePath, string objectPath, string objectKind = "Block", string filter = "AllObjects")
        {
            ...
            {
                var fallback = TryBuildCrossReferencesFromOfflineExports(softwarePath, objectPath, objectKind, filter);
                if (fallback != null)
                {
                    _logger?.LogInformation(
                        "GetCrossReferences fallback used on TIA V{Version}. softwarePath={SoftwarePath} objectPath={ObjectPath} objectKind={ObjectKind} count={Count}",
                ...
                _logger?.LogWarning("GetCrossReferences fallback returned no evidence on TIA V{Version}. softwarePath={SoftwarePath} objectPath={ObjectPath}", Engineering.TiaMajorVersion, softwarePath, objectPath);
                return new List<ModelContextProtocol.CrossReferenceEntry>();
            }
        ...
        private List<ModelContextProtocol.CrossReferenceEntry>? TryBuildCrossReferencesFromOfflineExports(
            string softwarePath,
            string objectPath,
            string objectKind,
            string filter)
        {
            try
            {
                if (!string.Equals(filter, "AllObjects", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(filter, "ObjectsWithReferences", StringComparison.OrdinalIgnoreCase))
                {
                    return new List<ModelContextProtocol.CrossReferenceEntry>();
                }

                var export = string.Equals(objectKind, "Type", StringComparison.OrdinalIgnoreCase)
                    ? ExportTypeToTemp(softwarePath, objectPath, preservePath: true)
                    : ExportBlockToTemp(softwarePath, objectPath, preservePath: true);
                if (export == null || export.Value.Paths == null || export.Value.Paths.Count == 0)
                    return null;

                var targetXml = export.Value.Paths.FirstOrDefault(x => File.Exists(x));
                if (string.IsNullOrWhiteSpace(targetXml) || !File.Exists(targetXml))
                    return null;

                var targetName = TryReadPlcXmlObjectName(targetXml!) ?? GetLeafName(objectPath);
                if (string.IsNullOrWhiteSpace(targetName))
                    return null;

                var regex = new Regex($@"(?<![A-Za-z0-9_]){Regex.Escape(targetName)}(?![A-Za-z0-9_])", RegexOptions.IgnoreCase | RegexOptions.Compiled);
                var items = new List<ModelContextProtocol.CrossReferenceEntry>();

                var candidateBlocks = GetBlocks(softwarePath, "");
                foreach (var block in candidateBlocks ?? new List<PlcBlock>())
                {
                    if (block == null) continue;
                    var candidatePath = BuildBlockPath(block);
                    if (string.IsNullOrWhiteSpace(candidatePath) || string.Equals(candidatePath, objectPath, StringComparison.OrdinalIgnoreCase))
                        continue;

                    TryAppendCrossReferenceEntriesFromCandidate(
                        items,
                        regex,
                        targetName,
                        objectPath,
                        candidatePath,
                        candidatePath,
                        () => ExportBlockToTemp(softwarePath, candidatePath, preservePath: true));
                }

                var candidateTypes = GetTypes(softwarePath, "");
                foreach (var type in candidateTypes ?? new List<PlcType>())
                {
                    if (type == null) continue;
                    var candidatePath = BuildTypePath(type);
                    if (string.IsNullOrWhiteSpace(candidatePath) || string.Equals(candidatePath, objectPath, StringComparison.OrdinalIgnoreCase))
                        continue;
                    ...
                    TryAppendCrossReferenceEntriesFromCandidate(
                        ...
                        candidatePath,
                        candidatePath,
                        () => ExportTypeToTemp(softwarePath, candidatePath, preservePath: true));
                }
            ...
            {
                _logger?.LogWarning(ex, "GetCrossReferences fallback failed. softwarePath={SoftwarePath} objectPath={ObjectPath} objectKind={ObjectKind}", softwarePath, objectPath, objectKind);
                return null;
            }
        }

        private void TryAppendCrossReferenceEntriesFromCandidate(
            List<ModelContextProtocol.CrossReferenceEntry> items,
            Regex regex,
            string targetName,
            string targetPath,
            string sourceName,
            string sourcePath,
            Func<(string TempDir, List<string> Paths)?> exportFactory)
        {
            try
            {
                var exported = exportFactory();
                if (exported == null || exported.Value.Paths == null || exported.Value.Paths.Count == 0)
                    return;

                var xmlPath = exported.Value.Paths.FirstOrDefault(x => File.Exists(x));
                if (string.IsNullOrWhiteSpace(xmlPath) || !File.Exists(xmlPath))
                    return;

                foreach (var evidence in ReadCrossReferenceEvidence(xmlPath!, regex).Take(20))
                {
                    items.Add(new ModelContextProtocol.CrossReferenceEntry
                    {
                        ...
                        LocationName = evidence.Item1,
                        ReferenceLocation = evidence.Item2,
                        ReferenceType = "OfflineExportScan",
                        Access = "ReadWrite"
                    });
                }
            }
            ...
            {
                _logger?.LogDebug(ex, "Skip fallback cross reference candidate sourcePath={SourcePath}", sourcePath);
            }
        }

        private static IEnumerable<Tuple<string, string>> ReadCrossReferenceEvidence(string xmlPath, Regex regex)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                var memberName = TryExtractAttributeValue(line, "Name")
                                 ?? TryExtractAttributeValue(line, "UId")
                                 ?? $"Line {i + 1}";
                var snippet = line.Trim();
                if (snippet.Length > 220)
                    snippet = snippet.Substring(0, 220);
                yield return Tuple.Create(memberName, $"Line {i + 1}: {snippet}");
            }
        }

        private static string? TryReadPlcXmlObjectName(string xmlPath)
        {
            try
            {
        ...
        private static string? TryExtractAttributeValue(string line, string attributeName)
        {
        ...
        private static string BuildBlockPath(PlcBlock block)
        {
        ...
        private static string BuildTypePath(PlcType type)
        {
        ...
        //   3) V17 strict XML compatibility: V17 rejects the generated <Namespace /> attribute
        //      on SW.Blocks.* and SW.Types.PlcStruct objects, while newer templates commonly
        //      include it. Strip only the exact SimaticML Namespace attribute element on V17.
        ...
        private string GetPlcTypeGroupPath(PlcTypeGroup group)
        {
            ...
            if (group is PlcTypeSystemGroup)
            {
                return string.Empty;
            }
        ...
        public ModelContextProtocol.ResponseDbMembers GetPlcDbMembers(string softwarePath, string blockPath, int maxDepth = 8, int maxMembers = 300)
        {
            var block = ResolveObject("Block", blockPath, softwarePath);
            if (block == null)
            {
                return new ModelContextProtocol.ResponseDbMembers
                {
                    Message = "Block not found",
                    ObjectKind = "Block",
                    ObjectPath = blockPath,
                    Members = Array.Empty<ModelContextProtocol.DbMemberInfo>()
                };
            }

            var blockType = block.GetType();
            var fullName = blockType.FullName ?? blockType.Name;
            if (!string.Equals(fullName, "Siemens.Engineering.SW.Blocks.GlobalDB", StringComparison.Ordinal) &&
                !string.Equals(fullName, "Siemens.Engineering.SW.Blocks.InstanceDB", StringComparison.Ordinal))
            {
                return new ModelContextProtocol.ResponseDbMembers
                {
                    Message = $"Target is not a DB block: {fullName}",
                    ObjectKind = "Block",
                    ObjectPath = blockPath,
                    TypeName = fullName,
                    Members = Array.Empty<ModelContextProtocol.DbMemberInfo>()
            ...
            var interfaceObj = GetPropertyPathValue(block, "Interface");
            if (interfaceObj == null)
            {
                return new ModelContextProtocol.ResponseDbMembers
                {
                    Message = "DB Interface property not found",
                    ObjectKind = "Block",
                    ObjectPath = blockPath,
                    TypeName = fullName,
                    Members = Array.Empty<ModelContextProtocol.DbMemberInfo>()
            ...
            var members = ReadDbMembersRecursive(interfaceObj, 0, Math.Max(1, maxDepth), Math.Max(1, maxMembers), new HashSet<object>(ReferenceEqualityComparer.Instance)).ToList();
            return new ModelContextProtocol.ResponseDbMembers
            {
                Message = "OK",
                ObjectKind = "Block",
                ObjectPath = blockPath,
                TypeName = interfaceObj.GetType().FullName ?? interfaceObj.GetType().Name,
                Members = members
            };
        }

        private static IEnumerable<ModelContextProtocol.DbMemberInfo> ReadDbMembersRecursive(object node, int depth, int maxDepth, int maxMembers, HashSet<object> visited)
        {
            if (node == null || depth >= maxDepth || !visited.Add(node)) yield break;

            foreach (var collectionName in new[] { "Sections", "Members", "MemberInfos", "Children", "Items" })
            {
                var collection = TryGetPropertyValue(node, collectionName);
                if (collection is not IEnumerable enumerable || collection is string) continue;

                var count = 0;
                foreach (var child in enumerable)
                {
                    if (child == null) continue;
                    if (count++ >= maxMembers) yield break;

                    var childType = child.GetType();
                    var name = TryGetName(child)
                        ?? TryGetPropertyValue(child, "Name")?.ToString()
                        ?? childType.Name;
                    var dataType = TryGetPropertyValue(child, "Datatype", "DataType", "TypeName", "Type")?.ToString();
                    var kind = TryGetPropertyValue(child, "SectionName", "Kind")?.ToString() ?? childType.Name;
                    var children = ReadDbMembersRecursive(child, depth + 1, maxDepth, maxMembers, visited).ToList();

                    yield return new ModelContextProtocol.DbMemberInfo
                    {
                        Name = name,
                        Kind = kind,
                        Type = dataType,
                        Signature = string.IsNullOrWhiteSpace(dataType) ? name : $"{name}: {dataType}",
                        Children = children
                    };
                }
            }
