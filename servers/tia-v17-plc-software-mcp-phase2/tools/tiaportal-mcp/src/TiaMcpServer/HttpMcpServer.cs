...
namespace TiaMcpServer
{
    ...
    internal static class HttpMcpServer
    {
        ...
        // Default upper bound on how long a POST waits for the MCP host to produce a
        // matching response before returning 504. TIA operations can be slow on V17,
        // so local product scripts can raise this through --http-response-timeout-seconds.
            ...
            Action<string> log)
        {
            ...
            var responseTimeout = TimeSpan.FromSeconds(Math.Max(30, options?.HttpResponseTimeoutSeconds ?? (int)DefaultResponseTimeout.TotalSeconds));
            ...
            log($"HTTP response timeout: {responseTimeout.TotalSeconds:0}s");
            if (secret == null)
            ...
            while (listener.IsListening)
            {
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        await Dispatch(ctx, secret, responseTimeout, requestLock, mcpWriter, mcpReader, log).ConfigureAwait(false);
                    }
                    catch (Exception ex)
                    {
