...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
        ...
        public static ResponseMessage EnsurePlcWatchTableEntry(
            [Description("softwarePath: path to the PLC software, e.g. 'PLC_1'")] string softwarePath,
            [Description("tableName: watch table name to create or update offline")] string tableName,
            [Description("address: variable address or symbolic tag to monitor, e.g. '%M10.0' or 'StartCmd'")] string address)
        {
            try
            {
                return Portal.EnsureWatchTableAddressEntry(softwarePath, tableName, address);
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw new McpException($"Unexpected error ensuring PLC watch table entry: {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
        ...
        public static ResponseMessage CreateTechnologyObject(
            ...
            string libVersion)
        {
            try { return Portal.CreateTechnologyObject(softwarePath, name, systemLibElement, libVersion); }
            catch (Exception ex) when (ex is not McpException)
            { throw new McpException($"Unexpected error creating technology object: {ex.Message}", ex, McpErrorCode.InternalError); }
        }
        ...
        [McpServerTool(Name = "GetPlcDbMembers"), Description("[L2][PLC-Software] Read a PLC DB block's interface member tree. Use this for GlobalDB / InstanceDB to inspect nested fields. Requires: Connect + OpenProject.")]
        public static ResponseDbMembers GetPlcDbMembers(
            [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath,
            [Description("blockPath: defines the path in the project structure to the DB block")] string blockPath,
            [Description("maxDepth: maximum recursion depth for nested members")] int maxDepth = 8,
            [Description("maxMembers: maximum members returned per level")] int maxMembers = 300)
        {
            try
            {
                return Portal.GetPlcDbMembers(softwarePath, blockPath, maxDepth, maxMembers);
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw new McpException($"Unexpected error reading DB members from '{blockPath}' in '{softwarePath}': {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
        ...
        [McpServerTool(Name = "ExportPlcMigrationBundle"), Description("[L2][PLC-Software][V17-Migration] Export selected PLC blocks and tag tables to a migration-ready XML bundle with a JSON manifest. Requires: Connect + OpenProject.")]
        public static ResponsePlcMigrationBundle ExportPlcMigrationBundle(
            [Description("softwarePath: path to the PLC software, e.g. 'PLC_1'.")] string softwarePath,
            [Description("outputDirectory: destination directory for the migration bundle. The tool creates blocks/, tagTables/, and manifest.json inside it.")] string outputDirectory,
            [Description("blockRegex: optional regex filter applied to block names. Empty means export all blocks.")] string blockRegex = "",
            [Description("preserveBlockPath: when true, preserve PLC block hierarchy inside blocks/.")] bool preserveBlockPath = false)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(outputDirectory))
                    throw new McpException("outputDirectory must not be empty", McpErrorCode.InvalidParams);

                Directory.CreateDirectory(outputDirectory);
                var blocksDirectory = Path.Combine(outputDirectory, "blocks");
                var tagTablesDirectory = Path.Combine(outputDirectory, "tagTables");
                Directory.CreateDirectory(blocksDirectory);
                Directory.CreateDirectory(tagTablesDirectory);

                var failed = new List<ImportFailure>();
                var exportedBlocks = new List<string>();
                var exportedTagTables = new List<string>();

                try
                {
                    var blockItems = Portal.ExportBlocks(softwarePath, blocksDirectory, blockRegex, preserveBlockPath);
                    exportedBlocks.AddRange(Directory.GetFiles(blocksDirectory, "*.xml", SearchOption.AllDirectories));
                    if ((blockItems?.Count() ?? 0) > 0 && exportedBlocks.Count == 0)
                    {
                        failed.Add(new ImportFailure
                        {
                            Path = blocksDirectory,
                            Error = "ExportBlocks returned blocks but no XML files were generated."
                        });
                    }
                }
                catch (Exception ex)
                {
                    failed.Add(new ImportFailure { Path = blocksDirectory, Error = $"Block export failed: {ex.Message}" });
                }

                try
                {
                    foreach (var tagTableName in Portal.GetPlcTagTables(softwarePath) ?? new List<string>())
                    {
                        var target = Path.Combine(tagTablesDirectory, MakeSafeFileName(tagTableName) + ".xml");
                        if (Portal.ExportPlcTagTable(softwarePath, tagTableName, target))
                        {
                            exportedTagTables.Add(target);
                        }
                        else
                        {
                            failed.Add(new ImportFailure { Path = target, Error = $"Tag table export failed: {tagTableName}" });
                        }
                    }
                }
                catch (Exception ex)
                {
                    failed.Add(new ImportFailure { Path = tagTablesDirectory, Error = $"Tag table export failed: {ex.Message}" });
                }
                ...
                var manifest = new JsonObject
                {
                    ...
                    ["preserveBlockPath"] = preserveBlockPath,
                    ["blocksDirectory"] = blocksDirectory,
                    ["tagTablesDirectory"] = tagTablesDirectory,
                    ["exportedBlocks"] = new JsonArray(exportedBlocks.Select(path => (JsonNode?)path).ToArray()),
                    ["exportedTagTables"] = new JsonArray(exportedTagTables.Select(path => (JsonNode?)path).ToArray()),
                    ["failed"] = new JsonArray(failed.Select(f => (JsonNode?)new JsonObject
                    {
                        ["path"] = f.Path,
                        ["error"] = f.Error
                    }).ToArray())
                ...
                return new ResponsePlcMigrationBundle
                {
                    Ok = ok,
                    OutputDirectory = outputDirectory,
                    BlocksDirectory = blocksDirectory,
                    ...
                    ExportedBlockCount = exportedBlocks.Count,
                    ExportedTagTableCount = exportedTagTables.Count,
                    ExportedBlocks = exportedBlocks,
                    ExportedTagTables = exportedTagTables,
                    Failed = failed,
                    Message = ok
                        ? $"PLC migration bundle exported: blocks={exportedBlocks.Count}, tagTables={exportedTagTables.Count}"
                        : $"PLC migration bundle exported with failures: blocks={exportedBlocks.Count}, tagTables={exportedTagTables.Count}, failed={failed.Count}",
                    Meta = new JsonObject
                    {
                        ["timestamp"] = DateTime.Now,
                        ["success"] = ok
            ...
            {
                throw new McpException($"Unexpected error exporting PLC migration bundle: {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
